...
pub use self::any::{Dimensions, TextureAnyImage};
pub use self::any::{TextureAny, TextureAnyLayer, TextureAnyLayerMipmap, TextureAnyMipmap};
pub use self::bindless::{BindlessTexturesNotSupportedError, ResidentTexture, TextureHandle};
pub use self::get_format::{GetFormatError, InternalFormat, InternalFormatType};
...
pub use self::ty_support::{is_cubemaps_supported, is_texture_2d_multisample_array_supported};
pub use self::ty_support::{is_texture_1d_array_supported, is_texture_3d_supported};
...
pub use crate::image_format::{
    UncompressedFloatFormat, UncompressedIntFormat, UncompressedUintFormat,
};
    ...
    fn num_levels(self, width: u32, height: Option<u32>, depth: Option<u32>) -> u32 {
        ...
        match self {
            ...
            EmptyMipmapsMax(i) | AutoGeneratedMipmapsMax(i) => {
                ...
                if i > max {
                    // TODO should we perform this check or just clamp the value?
...
pub trait Texture1dDataSink<T> {
    ...
    fn from_raw(data: Cow<'_, [T]>, width: u32) -> Self
    where
        [T]: ToOwned;
}
...
impl<'a, P: PixelValue> Texture1dDataSource<'a> for Vec<P>
where
...
impl<P> Texture1dDataSink<P> for Vec<P>
where
    P: Copy + Clone + Send,
{
    #[inline]
...
impl<'a, P: PixelValue> Texture1dDataSource<'a> for &'a [P]
where
    P: Copy + Clone + Send + 'static,
{
    type Data = P;
...
impl ToClientFormat for i8 {
    ...
    fn rgba_format() -> ClientFormat {
        ClientFormat::I8I8I8I8
    }
}

impl ToClientFormat for u16 {
    fn rgb_format() -> ClientFormat {
        ClientFormat::U16U16U16
    }
    fn rgba_format() -> ClientFormat {
        ClientFormat::U16U16U16U16
    }
}

impl ToClientFormat for i16 {
    fn rgb_format() -> ClientFormat {
        ClientFormat::I16I16I16
    }
    fn rgba_format() -> ClientFormat {
        ClientFormat::I16I16I16I16
    }
}
...
pub trait Texture3dDataSink<T> {
    /// Builds a new object from raw data.
    fn from_raw(data: Cow<'_, [T]>, width: u32, height: u32, depth: u32) -> Self
    where
        [T]: ToOwned;
}
...
impl<P> Texture3dDataSink<P> for Vec<Vec<Vec<P>>>
where
    P: Copy + Clone,
{
    #[inline]
